...
const MoviesList = (props) => {
    ...
    useEffect(() => {
        const GetMovies = async () => {
            ...
            try {
                const response = await axios.get(`http://localhost:5000/movies/${status[props.path].API}`, {
                },
                {
    ...
    return (
        ...
        <div className='movies-container'>
            <div className='movies-header'>
                ...
                <Link to={status[props.path].OtherPath}><h3>{status[props.path].OtherPathName}</h3></Link>
            </div>
