            ...
            try {
                ...
                const response = await axios.post(
                    `http://localhost:5000/book/schedule`,
                    {
                        "id_movie": 1,
                        "date": "2023-05-24",
                        "province": provinces.length > 0 ? provinces[provinceOption].province: "Hồ Chí Minh",
                    },
                    {
                    withCredentials: true,
                    }
                ...
                data.sort((a,b) => {
                    if (a.id_cinema < b.id_cinema || (a.id_cinema === b.id_cinema && a.id_room < b.id_room)) return -1
                    if (a.id_cinema > b.id_cinema || (a.id_cinema === b.id_cinema && a.id_room > b.id_room)) return 1
                    return 0
                })
                console.log(data)
  ...
  return (
    <>
        <div className='dates-container'>
            {dates !== 0 && dates.map((date, index) => (
                <div className={'date-container ' + (index === dateOption ? 'selected' :'')} key={index} onClick={() => {setDateOption(index)}}>
                    <div>
        ...
        <div className='provinces-container'>
            {provinces !== 0 && provinces.map((province, index) => (
                <div className = {'province-container ' + (index === provinceOption? 'selected' : '')} key={index} onClick={() => {setProvinceOption(index)}}>
                    {province.province}
                </div>
            ))}
        </div>
                ...
                return (
                    ...
                    <div className='cinema' key={cinema.id}>
                        <h5 className='cinema-name'>{cinema.cinema_name}</h5>
                            ...
                            {cinema.time.map(tm => {
                                return(
                                    <Link to={`/book/seats?id_schedule=${cinema.id}`} className='time-container' key={tm}>
                                        {tm}
                                    </Link>
                                )
                            })}
